;2018/08/30
;; 20180830 I know that something is wrong with my FAST coord conversions (see
;; JOURNAL__20180830__COMPARE_MY_FAST_COORD_CONVERSIONS_WITH_SDT_OUTPUT), so need to investigate
...
PRO JOURNAL__20180830__CONVERT_FAST_GSE_POS_AND_GEI_BFIELD_TO_OTHER_COORDINATES
  ...
  timeStrFile      = "FAST_dbFac-orb_"+STRING(FORMAT='(I0)',FASTorbit)+"-timeStrs.sav"
  outfile          = "FAST_dbFac-orb_"+STRING(FORMAT='(I0)',FASTorbit)+"-converted_coords.sav"

  RESTORE,outDir+FASTfile
  ...
  time_epoch    = UTC_TO_CDF_EPOCH(dB_fac.time)

  IF FILE_TEST(outDir+timeStrFile) THEN BEGIN
     PRINT,"Restoring timeString file " + outDir+timeStrFile + ' ...'
     RESTORE,outDir+timeStrFile
  ENDIF ELSE BEGIN
     PRINT,"Making " + STRCOMPRESS(N_ELEMENTS(dB_fac.time),/REMOVE_ALL) + " time strings ..."
     timeStr       = TIME_TO_STR(dB_fac.time,/MS)
  ...
  FOR i=0,nTot-1 DO BEGIN
     ...
     b_x_GEI     = FASTB_GEI.x[i]
     b_y_GEI     = FASTB_GEI.y[i]
     b_z_GEI     = FASTB_GEI.z[i]
     ...
     ;; GEOPACK_BCARSP_08  ,pos_x_GEI,pos_x_GEO, $
     ;;                     b_x_GE,b_theta_GEO,b_phi_GEO, $
     ;;                     b_x_GEO,b_y_GEO,b_z_GEO ; ,EPOCH=time_epoch[i]
