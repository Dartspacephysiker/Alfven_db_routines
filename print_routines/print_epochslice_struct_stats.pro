PRO PRINT_EPOCHSLICE_STRUCT_STATS,ssa, $
                                  PRINT_UNLOGGED=print_unlogged, $
                                  LUN=lun
  ...
  IF KEYWORD_SET(print_unlogged) THEN BEGIN
     FOR i=0,N_ELEMENTS(ssa)-1 DO BEGIN
        
        PRINTF,lun,FORMAT='("***UNLOGGED ",I4," : ",A0,"***")',i,ssa[i].dataName
        PRINTF,lun,FORMAT='(A8,T10,A8,T20,I0,T30,G9.4,T40,G9.4,T50,G9.4,T60,G9.4,T70,G9.4)', $
               TIME_TO_STR(ssa[i].eStart), $
               TIME_TO_STR(ssa[i].eEnd), $
               N_ELEMENTS(ssa[i].yList[0]), $
               10.^(ssa[i].moments.(0)), $
               10.^(SQRT(ssa[i].moments.(1))), $
               10.^(ssa[i].moments.(2)), $
               10.^(ssa[i].moments.(3)), $
               10.^(ssa[i].moments.(4)[1])
