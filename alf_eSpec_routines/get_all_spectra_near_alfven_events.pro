...
PRO GET_ALL_SPECTRA_NEAR_ALFVEN_EVENTS
  ...
  outFile           = (newellDBFile.SUBSTRING(0,-15)) + '--all_good_alf_eSpec_i--' + GET_TODAY_STRING(/DO_YYYYMMDD_FMT) + '.sav'
  eSpec             = !NULL
  ...
  outside_i__before = !NULL
  inside_i          = !NULL
  outside_i__after  = !NULL

  all_eSpec_for_good_alf__i = MAKE_ARRAY(nToCheck,/OBJ)
  all_eSpec_for_good_alf__types = MAKE_ARRAY(nToCheck,/OBJ)
  ...
  FOR i=0,nToCheck-1 DO BEGIN
     ...
     IF (nIn OR nOutBef OR nOutAft) GT 0 THEN BEGIN
        ...
        IF nOutBef GT 0 THEN BEGIN
           tmpOutBefType = MAKE_ARRAY(nOutBef,VALUE=1)
           types = [types,tmpOutBefType]
        ENDIF
        IF nIn GT 0 THEN BEGIN
           tmpInType = MAKE_ARRAY(nIn,VALUE=2)
           types = [types,tmpInType]
        ENDIF
        IF nOutAft GT 0 THEN BEGIN
           tmpOutAftType = MAKE_ARRAY(nOutAft,VALUE=3)
           types = [types,tmpOutAftType]
        ENDIF

        all_eSpec_for_good_alf__types[i] = LIST(types)
        all_eSpec_for_good_alf__i[i] = LIST(tmpKeep_i)
     ENDIF ELSE BEGIN
        nBad++
  ...
  PRINT,'Head checking...'
  FOR i=0,nToCheck-1 DO BEGIN
     IF ~ISA(all_eSpec_for_good_alf__i[i]) THEN STOP
  ENDFOR

  PRINT,'Saving all these wonderful indices...'
  SAVE,all_eSpec_for_good_alf__i,all_eSpec_for_good_alf__types, $
       FILENAME=NewellDBDir+NewellDBFile+outFile
END
