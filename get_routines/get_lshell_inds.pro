FUNCTION GET_LSHELL_INDS,maximus,minL,maxL,hemi,N_LSHELL=n_lshell,N_NOT_LSHELL=n_not_lshell,LUN=lun
  ...
  ;;       ;; lshell_i=where(maximus.lshell GE -ABS(maxL) AND maximus.lshell LE -ABS(minL) AND maximus.mlt GE minM AND maximus.mlt LE maxM) 
  ;;       lshell_i=where(maximus.lshell GE -ABS(maxL) AND maximus.lshell LE -ABS(minL),n_lshell,NCOMPLEMENT=n_not_LSHELL) 
  ;;       PRINTF,lun,'Hemisphere: Southern'
  ;;    ENDIF ELSE BEGIN
  ;;       IF STRUPCASE(hemi) EQ "NORTH" THEN BEGIN
  ;;          ;; lshell_i=where(maximus.lshell GE ABS(minL) AND maximus.lshell LE ABS(maxL) AND maximus.mlt GE minM AND maximus.mlt LE maxM) 
  ;;          lshell_i=where(maximus.lshell GE ABS(minL) AND maximus.lshell LE ABS(maxL),n_lshell,NCOMPLEMENT=n_not_LSHELL) 
  ;;          PRINTF,lun,'Hemisphere: Northern'
  ;;       ;; ENDIF ELSE lshell_i=where(maximus.lshell GE minL AND maximus.lshell LE maxL AND maximus.mlt GE minM AND maximus.mlt LE maxM) 
  ;;       ENDIF ELSE BEGIN
