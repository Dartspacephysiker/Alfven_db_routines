;+
; NAME:                           HISTOPLOT_ALFVENDBQUANTITIES_INDICES__OVERLAID
;
; PURPOSE:                        Plot histograms of user-supplied sets of indices
;
; OPTIONAL INPUTS:
;
; KEYWORD PARAMETERS:          MAXIND            : Index into maximus structure; plot corresponding quantity as a function of time
...
; EXAMPLE:
;
; MODIFICATION HISTORY:   2016/04/26 Barnebarn, ripped off HISTOPLOT_ALFVENDBQUANTITIES_DURING_STORMPHASES__OVERLAID
;-
PRO HISTOPLOT_ALFVENDBQUANTITIES__INDICES__OVERLAID, $
   indices_list, $
...
EPOCHPLOT_COLORNAMES=epochPlot_colorNames,SCATTEROUTPREFIX=scatterOutPrefix, $
  ...
  ;;Some setup stuff
  bad_input                     = 0
  CASE SIZE(indices_list,/TYPE) OF
     0: BEGIN
        bad_input               = 1
     END
     11: BEGIN
        IF TYPENAME(indices_list) EQ 'LIST' THEN BEGIN
           alf_i_list           = indices_list[*]
           nHistos              = N_ELEMENTS(alf_i_list)
           FOR i=0,nHistos-1 DO BEGIN
              nOrig             = N_ELEMENTS(alf_i_list[i])
              alf_i_list[i]     = CGSETINTERSECTION(alf_i_list[i],good_i)
              nAfter            = N_ELEMENTS(alf_i_list[i])
              PRINT,'Dropped ' + STRCOMPRESS(nOrig-nAfter,/REMOVE_ALL) + ' of provided indices after good_i combination'
           ENDFOR
        ENDIF ELSE BEGIN
           bad_input            = 1
        ENDELSE
     END
  ENDCASE
  IF bad_input THEN BEGIN
     PRINT,"Must provide input indices in the form of a list! (e.g., ind_list = LIST(inds1,inds2,...)"
     PRINT,'Quitting...'
     RETURN
  ENDIF

  suff        = KEYWORD_SET(suff_dataname) ? suff_dataname : ''
  strings     = KEYWORD_SET(names_for_sets_of_inds) ? names_for_sets_of_inds : STRING("histoplot_inds_" + STRCOMPRESS(INDGEN(nHistos),/REMOVE_ALL))
  niceStrings = KEYWORD_SET(fancyNames_For_sets_of_inds) ? fancyNames_For_sets_of_inds : STRING("Histoplot index set #" + STRCOMPRESS(INDGEN(nHistos),/REMOVE_ALL))
  ...
  ;;data out
  genFile_pref    = date + '--' + dataName + '--from_histoplot_alfvendbquantities__indices__overlaid.pro'
  outstats        = date + '--' + dataName + '_moment_data_from_'+GET_TODAY_STRING(/DO_YYYYMMDD_FMT)+'.sav'

  SET_PLOT_DIR,plotDir,/FOR_ALFVENDB,/VERBOSE,/ADD_TODAY

  saveName        = plotDir + 'alfvendb_histos--overlaid--' + dataName + tempSuffix + '.png' ;savePlotSuffix
  ...
  FOR i=0,nHistos-1 DO BEGIN
     ...
     IF KEYWORD_SET(normalize_maxInd_hist) THEN BEGIN
        y              = y / integral
        pHP.yRange     = KEYWORD_SET(histYRange_maxInd) ? histYRange_maxInd : [0,0.5]
