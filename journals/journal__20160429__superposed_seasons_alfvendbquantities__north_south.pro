PRO JOURNAL__20160429__SUPERPOSED_SEASONS_ALFVENDBQUANTITIES__NORTH_SOUTH
  ...
  ;; maxM                  = [18.0,18.0]
  ;; MLTstr                = '(Dayside)'
  ;; mltSimpleStr          = '--dayside'

  minM                  = [00.0,00.0]
  maxM                  = [24.0,24.0]
  MLTstr                = '(All MLTs)'
  mltSimpleStr          = '--all_MLTs'

  ;; minM                  = [-6.0,-6.0]
  ;; maxM                  = [06.0,06.0]
  ;; MLTstr                = '(Nightside)'
  ;; mltSimpleStr          = '--nightside'
  ...
  hemi                  = ['NORTH','SOUTH']
  ptRegion              = ['North','South'] + ' ' + MLTstr

  psRegion              = ['north','south']
  symColor              = ['red','blue']
  ;; ptRegion              = ['Both sides','Dayside','Nightside']
  ;; psRegion              = ['bothSides','dayside','nightside']
  ;; symColor              = ['black','red','blue']

  pref                  = 'journal_20160429--SEA_seasons--NORTH_SOUTH_comparison'+mltSimpleStr
  ...
  IF do_despun THEN BEGIN
     plotSuff       = '--despun_db.png'
  ENDIF ELSE BEGIN
     plotSuff       = '.png'
  ENDELSE

  CASE 1 OF
     KEYWORD_SET(probOccurrence_SEA): dataName = 'probOccurrence'
     KEYWORD_SET(nEventHists):        dataName = 'nEventHists'
  ENDCASE

  probOccPref       = pref + '--' + dataName + $
