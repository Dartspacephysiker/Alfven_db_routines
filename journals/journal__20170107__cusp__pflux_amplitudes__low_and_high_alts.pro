;;01/05/17
PRO JOURNAL__20170107__CUSP__PFLUX_AMPLITUDES__LOW_AND_HIGH_ALTS
  ...
  psFileName = 'Altitude_vs_Alf_pFlux' + $
  ...
  data = MAXIMUS__maximus.pFluxEst
  IF KEYWORD_SET(absMagC) THEN BEGIN
     data = ABS(data)
     titlePref = ""
     xTitle    = "Poynting flux (mW/m^2)"
  ENDIF ELSE BEGIN
     titlePref = ""
  ...
  WHILE alt LT altMax DO BEGIN
     ...
     ;;                                        maximus__maximus.alt LT (alt+altDelta) AND maximus__maximus.pFluxEst GE 10))
     tmpI    = CGSETINTERSECTION(inds,WHERE(maximus__maximus.alt GE alt AND $
                                            maximus__maximus.alt LT (alt+altDelta) AND maximus__maximus.pFluxEst GE 10))
  ...
  title = STRING(FORMAT='(F0.1,"-",F0.1," MLT, ",F0.1,"-",F0.1," ILAT")', $
                 minM,maxM, $
                 minI,maxI)
  plot = PLOT(ABS(maximus__maximus.pFluxEst[inds]), $
              ...
              TITLE=title, $
              YTITLE='Altitude (km)', $
              XTITLE='Poynting Flux ($mW/m!U2!N)')
  ;; plot = PLOT(meanMag, $
  ;;             altArr, $
  ;;             COLOR='Red', $
  ;;             THICK=2, $
  ...
  ;; plotMean = PLOT,10.^(wCenter),meanFAC, $
  PLOT,10.^(wCenter),meanFAC, $
                  /XLOG, $
                  XTITLE='FAC width (m)', $
                  YTITLE='Mean FAC Amplitude ($\mu$A/m!U2!N)'

  ;; plotMed = PLOT,10.^(wCenter),medFAC, $
  PLOT,10.^(wCenter),medFAC, $
                  /XLOG, $
                  XTITLE='FAC width (m)', $
                  YTITLE='Median FAC Amplitude ($\mu$A/m!U2!N)'

  ;; plotMax = PLOT,10.^(wCenter),maxFAC, $
  PLOT,10.^(wCenter),maxFAC, $
                  /XLOG, $
                  XTITLE='FAC width (m)', $
